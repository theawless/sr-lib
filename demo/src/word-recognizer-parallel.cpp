...
public:
	WordRecognizerParallel(string folder, vector<string> audio_names, int n_total_utterances, int n_train_utterances) :
		audio_processor(AudioProcessor(320, 80)), mfcc(MFCC()), codebook(Codebook(M)), thread_pool(4 * thread::hardware_concurrency()),
...
int main()
{
	...
	WordRecognizerParallel word_recognizer(folder, audio_names, n_total_utterances, n_train_utterances);
